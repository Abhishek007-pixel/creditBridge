        ...
        conn.execute(
            """INSERT INTO credit_scores
               (id, applicant_id, phone_score, ecommerce_score, geo_score,
                psychometric_score, merchant_score, cashflow_score, financial_commitment_score,
                final_score, risk_category, loan_recommended, interest_rate,
                explanation, phone_reason, ecommerce_reason, geo_reason,
                psychometric_reason, merchant_reason, cashflow_reason, financial_commitment_reason,
                weights_used, pipeline_mode)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                new_id(),
                req.applicant_id,
                breakdown.get("bill_consistency", {}).get("score", breakdown.get("phone_bill", {}).get("score", 0)),
                ...
                breakdown.get("financial_commitment", {}).get("score", 0),
                result.get("final_score", 0),
                result.get("risk_category", ""),
                result.get("loan_recommended", 0),
                result.get("interest_rate", 0.0),
                result.get("explanation", ""),
                breakdown.get("bill_consistency", {}).get("reason", breakdown.get("phone_bill", {}).get("reason", "")),
                ...
                breakdown.get("financial_commitment", {}).get("reason", ""),
